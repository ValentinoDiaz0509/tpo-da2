...
public interface TelemetryReadingRepository extends JpaRepository<TelemetryReading, UUID> {
    ...
    @Query("""
        ...
        AND t.recordedAt = (
            SELECT MAX(t2.recordedAt) FROM TelemetryReading t2 WHERE t2.patient.id = t.patient.id
        )
        """)
    List<TelemetryReading> findLatestReadingsForPatients(@Param("patientIds") List<UUID> patientIds);
